...
if (isset($_POST['btnAddress'])) {
    ...
    if ($fullname !== "" && $phone_number !== "" && $city !== "" && $postal_code !== "" && $full_address !== "") {  
        ...
        if ($res->num_rows > 0) {
            $sql = "INSERT INTO user_addresses (user_id, fullname, city, full_address, postal_code, phone_number, is_default, created_at) VALUES ('$user_id', '$fullname', '$city', '$full_address', '$postal_code', '$phone_number', 0 ,'$createdAt')";
            $result = mysqli_query($conn, $sql);
        } else {
            $sql = "INSERT INTO user_addresses (user_id, fullname, city, full_address, postal_code, phone_number, is_default, created_at) VALUES ('$user_id', '$fullname', '$city', '$full_address', '$postal_code', '$phone_number',  1 ,'$createdAt')";
            $result = mysqli_query($conn, $sql);
        }
